...
vec3 fauna_model::get_bird_position(float t, float t0, float t1, float t2, float t3, const vec3& p0, const vec3& p1, const vec3& p2, const vec3& p3, float K)
{
...
vec3 fauna_model::get_bird_position(float t) {
    // ********************************************* //
// Compute interpolated position at time t
// ********************************************* //
    const int idx = index_at_value(t);
    ...
    return get_bird_position(t, t0, t1, t2, t3, p0, p1, p2, p3, 0.5f);

}

mat3 fauna_model::get_bird_rotation(float t, float t0, float t1, float t2, float t3, const vec3& p0, const vec3& p1, const vec3& p2, const vec3& p3, float K)
{
    const vec3 d1 = 2 * K * (p2 - p0) / (t2 - t0);
    const vec3 d2 = 2 * K * (p3 - p1) / (t3 - t1);
...
mat3 fauna_model::get_bird_rotation(float t)
{
    // ********************************************* //
...
// ********************************************* //
    ...
    return get_bird_rotation(t, t0, t1, t2, t3, p0, p1, p2, p3, 0.2f);

}


size_t fauna_model::index_at_value(float t)
{
    const size_t N = keyframes.size();
    

    size_t k = 0;
    while (keyframes[k + 1].t < t)
        ++k;
    return k;
}

void fauna_model::set_keyframes()
{
...
std::vector<vec3> keyframe_position = { {54.9148f,19.3474f,30.7046f}, {67.6399f,28.4372f,26.3839f},
    ...
    for (size_t i = 0; i < keyframe_position.size(); i++)
    {
        ...
        keyframes.push_back({ keyframe_position[i], t });
        if (i != keyframe_position.size() - 1)
            t += norm(p - keyframe_position[i + 1]) / speed;
    }

    timer_fauna.t_min = keyframes[1].t;                   // first time of the keyframe
    timer_fauna.t_max = keyframes[keyframes.size() - 2].t;  // last time of the keyframe
    timer_fauna.t = timer_fauna.t_min;
...
void fauna_model::draw_keyframes(std::map<std::string, GLuint>& shaders, scene_structure& scene)
{
...
void fauna_model::mouse_move(scene_structure& scene, GLFWwindow* window)
{
    ...
    if (mouse_click_left && key_shift && picked_object != -1)
    {
        ...
        if (picked_object == 0 && N > 2)
            keyframes[N - 3].p = info.intersection;
        else if (picked_object == 1 && N > 1)
            keyframes[N - 2].p = info.intersection;
        else if (picked_object == 2 && N > 0)
            keyframes[N - 1].p = info.intersection;
        else if (N > 2 && picked_object == N - 2)
            keyframes[0].p = info.intersection;
        else if (N > 1 && picked_object == N - 1)
            keyframes[1].p = info.intersection;
